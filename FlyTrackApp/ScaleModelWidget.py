...
class ScaleModelWidget(pg.GraphicsWindow):
	...
	def update_table(self):
		for i in range(len(self.lengths)):
			self.table_item.setItem(1,i,QTableWidgetItem(str(self.lengths[i])))
	
	def update_model_scale(self):
		#self.flt.set_model_scale(self.lengths)
		body_length = self.lengths[0]+self.lengths[2]
		self.flt.set_body_length(body_length)
		wing_length = (self.lengths[3]+self.lengths[4])/2.0
		self.flt.set_wing_length(wing_length)
		self.flt.set_model_scale([wing_length,wing_length,wing_length,wing_length,wing_length])
		x_0 = self.xyz_pos[0,0]
		y_0 = self.xyz_pos[0,1]
		z_0 = self.xyz_pos[0,2]
		self.flt.set_model_origin(x_0,y_0,z_0)